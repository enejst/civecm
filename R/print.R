...
print.summary.I1 <- function(obj, ...) {
  cat('\nCall:\n')
  cat(paste(deparse(obj$call), sep = "\n", collapse = "\n"), "\n", sep = "")
  
  periodX <- periodicity(obj$X)
  periodZ <- periodicity(obj$Z0)
  cat('\nEffective Sample: ',
  ...
  if (ncol(obj$beta) == 0)
    ...
    cat('\nalpha:\n')
    print(coefmatrix(obj$alpha, obj$t.alpha), right = TRUE, quote = FALSE)
    
    cat('\nPi:\n')
    print(coefmatrix(obj$Pi, obj$t.Pi), right = TRUE, quote = FALSE)
  }
