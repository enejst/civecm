...
c
      subroutine johval(npr,itt,itv,nc,isave,arg,val,rpath,nrpath)
c
...
c     Andreas, 2010-04-21: Some adjustment so no variables are
c     declared implicitly
      implicit none
      integer 			:: npr,itt,itv,nc,isave,i,ii,iskip,np,nx,nrpath
      double precision	:: arg, val, junk, precrt, size, pval, stat, cval
      character(len=255):: rpath
c
      character(len=12)	:: fnames(12)
      double precision	:: probs(221), cnorm(221), crits(221), wght(221)
...
c
c     Andreas, 1010-04-21: My own adustments of the original file 
c     to ease the interaction with R
c
      open (unit=12,file = rpath(:nrpath) // '/extdata/probs.tab',
     &  status='OLD',err=602)
  601 continue
...
c
      open (unit=11,
     & file=rpath(:nrpath)//'/extdata/'//fnames(npr),status='OLD',
     &	err=605)
  604 continue
c
